(function($) {
    ...
    // Notice Hide
    $('body').on('click', '.jltflickr-upgrade-popup .popup-dismiss', function(evt) {
        evt.preventDefault();
        $(this).closest('.jltflickr-upgrade-popup').fadeOut(200);
    });

    // Notice Show
    $('body').on('click', '.disabled', function(evt) {
        evt.preventDefault();
        $('.jltflickr-upgrade-popup').fadeIn(200);
    });
    ...
    // Search
    $('.jltflickr-search-plugins #search-plugins').on('keyup', function() {
        var value = $(this).val();
...
function activatePlugin(element, file) {
    ...
    jQuery.ajax({
        url: JLTFLICKRCORE.admin_ajax,
        type: "POST",
        data: {
            action: "jlt_flickr_recommended_activate_plugin",
            file: file,
            nonce: JLTFLICKRCORE.recommended_nonce
        },
...
function installPlugin(element, plugin) {
    ...
    jQuery.ajax({
        ...
        type: "POST",
        data: {
            action: "jlt_flickr_recommended_upgrade_plugin",
            type: 'install',
            plugin: plugin,
            nonce: JLTFLICKRCORE.recommended_nonce
        },
